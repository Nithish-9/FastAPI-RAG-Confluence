...
def _load_language(name: str) -> Optional[Language]:
    """
    Load a tree-sitter Language from tree-sitter-language-pack.
    The pack uses its own language name aliases.
    Returns None if the language is unavailable.
...
LANGUAGE_NODE_TYPES: dict[str, list[str]] = {
    ...
    "html":       ["element"],
    "css":        ["rule_set", "at_rule", "media_statement"],
    "sql":        ["create_table_statement", "create_function_statement",
...
def _treesitter_chunks(
    ...
    Uses the tree-sitter 0.25 API: Parser(Language(...)).
    """
    ...
    # Capture top-level code not covered by any symbol (imports, globals, etc.)
    if collected_ranges:
        # Sort ranges and extract gaps — O(n log n) not O(file_size)
        ...
        if leftover_text and len(leftover_text) > 20:
            results.insert(0, (leftover_text, "module-level", 0, 0))
...
class CodeParser:
    ...
    def _treesitter_parse(
        ...
        if not raw_results:
            return self._fallback_chunks(file_content, file_name, file_extension,
                                         file_path, workspace_path, lang)
        raw_chunks: list[Tuple[str, Optional[str]]] = [(text, symbol) for text, symbol, _, _ in raw_results]
    ...
    def _fallback_chunks(
        ...
        raw_chunks = [(s, None) for s in splits if s.strip()]
        return self._build_chunks(raw_chunks, file_name, file_extension,
                                  file_path, workspace_path, lang)
